...
private struct HabiticaTaskEnvelope: Decodable {
    let data: [HabiticaTaskSummary]
}

private struct HabiticaTaskSummary: Decodable {
    let id: String
    let alias: String?
...
private struct HabiticaEmptyEnvelope: Decodable {
    let success: Bool?
}

private enum HabiticaError: Error, LocalizedError {
    case missingAlias
